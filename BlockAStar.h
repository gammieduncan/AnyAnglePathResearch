...
private:
	...
	void InitializeBlock(CornerLoc l);
	void ResetSearch();
	...
	cost BlockAStarSearch(CornerLoc l1, CornerLoc l2, std::vector<CornerLoc> & thePath);

	bool OnHorizontalBlockEdge(CornerLoc l){return l.x%block_cell_width_ == 0;}
	bool OnVerticalBlockEdge(CornerLoc l){return l.y%block_cell_width_ == 0;}
	bool OnBlockEdge(CornerLoc l){return OnHorizontalBlockEdge(l) || OnVerticalBlockEdge(l);}
	bool OnBlockEdge(cornerId c){return OnBlockEdge(corner_locations_[c]);}
	bool OnBlockCorner(CornerLoc l){return OnHorizontalBlockEdge(l) && OnVerticalBlockEdge(l);}
	bool OnBlockCorner(cornerId c){return OnBlockCorner(corner_locations_[c]);}

	cost FindPathWithinBlock(blockId b, cornerId start, cornerId goal, std::vector<CornerLoc> &path);
	...
	struct Block{
		Block(CornerLoc l) :top_left(l){dist = NULL; next = NULL; last_g_val = NULL;}
		CornerLoc top_left;	// Its top-left corner marks the location of the block
