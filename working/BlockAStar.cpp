...
void BlockAStar::BlockAStarInitialize()
{
	...
	for(unsigned int x = 0; x < map_block_width_; x++)
		for (unsigned int y = 0; y < map_block_height_; y++)
			InitializeBlock(CornerLoc(x*block_cell_width_, y*block_cell_width_));	// Initialize the block specified by the top left corner and add it to the blocks list
...
void BlockAStar::InitializeBlock(CornerLoc l)
{
...
cost BlockAStar::FindPathWithinBlock(blockId b, cornerId start, cornerId goal, std::vector<CornerLoc> & thePath)
{
...
cost BlockAStar::FindCornerLocPath(xyLoc from, xyLoc to, std::vector<CornerLoc> &thePath)
{
	thePath.clear();
	...
	if (start == goal){
		thePath.push_back(ToCornerLoc(from));
		return 0;
	}
