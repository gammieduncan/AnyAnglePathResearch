...
const ANYAStateKey ANYA::GenerateKey(const CornerLoc root, const Interval interval) const
{
...
const stateId ANYA::GenerateState(const CornerLoc root, const Interval interval, const CornerLoc goal)
{
...
void ANYA::GenerateSubIntervals(const int row, const double left, const double right, const CornerLoc root, const CornerLoc goal, std::vector<stateId> & ids)
{
...
void ANYA::GenerateIntervalsTowardsLeft(const double fx, const int y, const int unblockedDir, const CornerLoc root, const CornerLoc goal,
...
void ANYA::GenerateIntervalsTowardsRight(const double fx, const int y, const int unblockedDir, const CornerLoc root, const CornerLoc goal,
        std::vector<stateId> & successors, const double bound)
{
    // Determine how far the set of intervals extend and then call GenerateSubIntervals(..)
...
void ANYA::GenerateSuccessors_SameRow(const CornerLoc root, const Interval interval, const CornerLoc goal, std::vector<stateId> & successors)
{
    ...
    if (root.x >= interval.i_right) {
        CornerLoc left_end_point(interval.i_left, interval.row);
    ...
    if (root.x <= interval.i_left) {
        CornerLoc right_end_point(interval.i_right, interval.row);
...
void ANYA::GenerateSuccessors_DifferentRow(const CornerLoc root, const Interval interval, const CornerLoc goal, std::vector<stateId> & successors)
{
    ...
    // TOWARDS LEFT (explained assuming that dir = TOP, that is, the root is towards the bottom of the 'interval'):
    ...
    // Let TL (Top-Left), TR (Top-Right), BL (Bottom-Left), BR (Bottom-Right) be the cells surrounding the left end-point of the 'interval'.

    // Some observations:
    // - BR cannot be blocked because otherwise 'interval' cannot be observable by the root.
    //
    // - TL cannot be blocked if either TR or BL is blocked, because, otherwise, the left end-point will not be at a convex corner of an obstacle.
    //
    // - We generate an interval in the same row iff BL is blocked.
    //
    // - If TL is blocked (and, therefore, TR is unblocked), then there are unobservable successors in the next row iff the root of the expanded state is
    // towards left of the left end-point of the interval of the expanded state. We start generating successors towards left, starting from 'intersecting_left'
    // and stop at the left end-point of the interval of the expanded state.
    //
    // - If TR is blocked (and, therefore, TL is unblocked), then we need to generate intervals in the next row towards left, starting from the
    // x- coordinate of the left end-point of the interval being expanded and stop at 'intersecting_left'.
    //
    // - If BL is blocked, then we need to generate intervals towards left in the new row, starting from the 'new_left' computed above.
    //
    // - If both TR and BL are blocked, then we ganerate the intervals

    // If the left end-point of the expanded interval is not a convex corner of an obstacle, we can't generate unobservable successors towards left.

    if (fabs(interval.f_left - (double) interval.i_left) < EPSILON && corners_[interval.i_left][interval.row].is_convex_corner) {
        CornerLoc new_root = CornerLoc(interval.i_left, interval.row);
        ...
        // BL is blocked: Generate a single successor interval in the same row.
        if (corners_[interval.i_left][interval.row].interval_blocked[rev_dir][LEFT])
            successors.push_back(GenerateState(new_root, GetLeftInterval(interval.i_left, interval.row), goal));

        // Both TR and BL are blocked (two diagonally adjacent obstacles): Generate intervals
        if (corners_[interval.i_left][interval.row].interval_blocked[dir][RIGHT] && corners_[interval.i_left][interval.row].interval_blocked[rev_dir][LEFT])
            GenerateIntervalsTowardsLeft(interval.i_left, new_row, rev_dir, new_root, goal, successors);
            ...
            if (corners_[interval.i_left][interval.row].interval_blocked[dir][LEFT]) // If the corner is from an obstacle between I's row and the next interval's row
                if (interval.i_left > root.x) // The root of the state should be towards the left of I for the path to be taut
                    GenerateIntervalsTowardsRight(interval.i_left, new_row, rev_dir, new_root, goal, successors, new_left);
            ...
            if (corners_[interval.i_left][interval.row].interval_blocked[rev_dir][LEFT]) // If the corner is from an obstacle that is away from the next interval's row
                if (new_left == intersecting_left)
                    GenerateIntervalsTowardsLeft(new_left, new_row, rev_dir, new_root, goal, successors);
        }
            ...
            corners_[interval.i_right][interval.row].is_convex_corner) // If the corresponding corner is a convex corner of an obstacle
    {
        if (corners_[interval.i_right][interval.row].interval_blocked[rev_dir][RIGHT]) // If turning that corner produces a taut path)
            successors.push_back(GenerateState(CornerLoc(interval.i_right, interval.row), GetRightInterval(interval.i_right, interval.row), goal));
        ...
        if (corners_[interval.i_right][interval.row].interval_blocked[dir][LEFT] && corners_[interval.i_right][interval.row].interval_blocked[rev_dir][RIGHT])
            GenerateIntervalsTowardsRight(interval.i_right, new_row, rev_dir, CornerLoc(interval.i_right, interval.row), goal, successors);
            ...
            if (corners_[interval.i_right][interval.row].interval_blocked[dir][RIGHT]) // If the corner is from an obstacle between I's row and the next interval's row
                if (interval.i_right < root.x) // The root of the state should be towards the right of I for the path to be taut
                    GenerateIntervalsTowardsLeft(interval.i_right, new_row, rev_dir, CornerLoc(interval.i_right, interval.row), goal, successors, new_right);
            ...
            if (corners_[interval.i_right][interval.row].interval_blocked[rev_dir][RIGHT]) // If the corner is from an obstacle that is away from the next interval's row
                if (new_right == intersecting_right)
                    GenerateIntervalsTowardsRight(new_right, new_row, rev_dir, CornerLoc(interval.i_right, interval.row), goal, successors);
        }
    }
}
void ANYA::GenerateSuccessors(const CornerLoc l, const Interval I, CornerLoc const goal, std::vector<stateId> & successors)
{
...
void ANYA::InitializeSearch(const CornerLoc from, const CornerLoc to)
{
    ResetSearch();
...
cost ANYA::FindCornerLocPath(xyLoc start_loc, xyLoc goal_loc, std::vector<CornerLoc> &path)
{
    path.clear();

    CornerLoc from = ToCornerLoc(start_loc);
    CornerLoc to = ToCornerLoc(goal_loc);
...
cost ANYA::HeuristicDistance(const CornerLoc root, const Interval interval, const CornerLoc goal) const
{
...
#ifdef ANY_ANGLE_RUNNING_IN_HOG
void ANYA::ProcessDebugLoc(CornerLoc l)
{
    printf("\nDebug point: (%d, %d)\n", l.x, l.y);
