...
public:
    ...
    void ProcessDebugLoc(const CornerLoc loc);
...
private:
    ...
    bool finalized_;    // Starts as false, set to true once FinalizeGraph() is called. Not necessary for the algorithm, but useful for printing debug info.
    ...
    void GetDirectHReachableSubgoals(const CornerLoc & from, std::vector<subgoalId> & subgoals);
    void FinalizeGraph();
    ...
    cost FindSubgoalAnyAnglePath(xyLoc l1, xyLoc l2, std::vector<CornerLoc> & thePath, bool theta = true);
    ...
    void ConnectStartAndGoalToGraph(CornerLoc & start, CornerLoc & goal, subgoalId & sgStart, subgoalId & sgGoal,
            std::vector<subgoalId> & startDirectHReachableSubgoals, std::vector<subgoalId> & goalDirectHReachableSubgoals);
    cost ExtractPath(subgoalId l1, subgoalId l2, std::vector<CornerLoc> &path);

    bool IsSubgoal(const cornerId id) const {return id != NO_CORNER && corners_[id].sg_id != NON_SUBGOAL;}
    bool IsSubgoal(const CornerLoc l) const {return IsSubgoal(ToCornerId(l));}

    subgoalId ToSubgoalId(const cornerId id) const  {return corners_[id].sg_id;}
    subgoalId ToSubgoalId(const CornerLoc l) const      {return ToSubgoalId(ToCornerId(l));}

    void SetClearance(const cornerId id, const direction d, const int c)    {corners_[id].clearance[d] = c;}
    int GetClearance(const cornerId id, const direction d) const    {return id == NO_CORNER ? 0 : corners_[id].clearance[d];}

    void SetJumpTo(const cornerId id, const direction d, const cornerId c){corners_[id].jump_to[d] = c;}
    cornerId GetJumpTo(const cornerId id, const direction d){return corners_[id].jump_to[d];}
