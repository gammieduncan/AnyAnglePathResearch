...
#ifdef ANY_ANGLE_RUNNING_IN_HOG
    debug_loc_ = CornerLoc(10000, 10000);
//    if (corner_locations_.size() > 0)
//        debug_loc_ = corner_locations_[0];
#endif
...
#else
    // TODO : Assumes that the given x and y coordinates map to an unblocked cell
    return cells_[x+1][y+1].northwest_corner;   // If it is a diagonal corner
#endif
}
cornerId AnyAngleAlgorithm::ToCornerId(const CornerLoc & loc) const
{
#ifndef DISALLOW_MOVES_BETWEEN_DIAGONAL_OBSTACLES
    return corner_ids_[loc.x][loc.y];
#else
    // TODO
    switch (loc.type) {
...
std::vector<cornerId> AnyAngleAlgorithm::GetNeighbors(const cornerId c) const
{
    ...
    for (int i = 0; i < 8; i++)
    {
        if (corner_neighbors_[c][i] != NO_CORNER)
            neighbors.push_back(corner_neighbors_[c][i]);
    }
...
cost AnyAngleAlgorithm::EvaluatePath(const std::vector<xyLocCont> &path, const bool validate_path)
{
    ...
    if (validate_path) {
        if (path.size() == 0) {
            ...
            // First and last locations on the path should be start and goal
            if (!(path[0] == ToCornerLoc(from_)))
                valid_path = false;

            if (!(path.back() == ToCornerLoc(to_)))
                valid_path = false;
        }
    }
...
void AnyAngleAlgorithm::ReportStatistics(const std::vector<CornerLoc> &path, AnyAngleStatistics* stats, const bool validate_path)
{
...
void AnyAngleAlgorithm::DrawPath(const MapEnvironment *env, const std::vector<CornerLoc> &path, const bool draw_mid_points, const int priority) const
{
    if (draw_mid_points)
