...
void FieldAStar::FieldAStarInitialize()
{
    ...
    // Neighbors in 8 different directions (ordered N, NE, E, ...). NO_CORNER if no corner exists in a direction.
     for (cornerId i = 0; i < corner_locations_.size(); i++)
     {
         corners_[i].neighbors = GetCornerNeighbors(i);
    ...
    for (cornerId c = 0; c < corner_locations_.size(); c++) {
        ...
        if (IsTraversable(NorthWestCell(l)) || IsTraversable(NorthEastCell(l)))
            ...
            corners_[c].neighbors.push_back(NO_CORNER);
        ...
        for (unsigned int i = 0; i < corners_[c].neighbors.size(); i++)
            if (corners_[c].neighbors[i] == NO_CORNER)
...
{
    // TODO: no movement between diagonal obstacles
    neighbors.clear();
    gValues.clear();
    exists.clear();
...
#ifdef DISALLOW_MOVES_BETWEEN_DIAGONAL_OBSTACLES
    if (xLocs[0] + 1 != xLocs[2])   // If l lies on a horizontal edge and is not a corner.
    {

    }
    else if (yLocs[0] + 1 != yLocs[2])  // If l lies on a vertical edge and is not a corner.
    {
    ...
    for (int d = 0; d < 8; d++)
        if (exists[d] && IsInteger(neighbors[d].x) && IsInteger(neighbors[d].y)) {
            CornerLoc l1 = neighbors[d].getCorner();
...
cost FieldAStar::ExtractPath(CornerLoc startLoc, CornerLoc goalLoc, std::vector<xyLocCont> & thePath)
{
    thePath.clear();
    cost pathCost = 0;
...
cost FieldAStar::FindXYLocContPath(xyLoc from, xyLoc to, std::vector<xyLocCont> &thePath)
{
        ...
        for (unsigned int d = 0; d < corners_[curr].neighbors.size(); d++) {
            ...
            if (succ != NO_CORNER) {
                ...
                if (corners_[succ].list != CLOSED_LIST) {
                    ...
                    if (cw != NO_CORNER) // If clockwise neighbor exists
                    {
                    ...
                    if (ccw != NO_CORNER) // If clockwise neighbor exists
                    {
    ...
    std::cout<<"Field A* gval: "<<corners_[goal].g_val<<std::endl;
    return corners_[goal].g_val;
//    return ExtractPath(ToCornerLoc(start), ToCornerLoc(goal), thePath);
}
