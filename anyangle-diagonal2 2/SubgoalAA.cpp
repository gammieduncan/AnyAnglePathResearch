...
{
	delta_map_loc_[DIR_N] = -width_ + 1;// North	(We have 'width - 1' number of corners in each row, so going north subtracts width - 1 from its current location)
...
void SubgoalAA::IdentifySubgoals()
{
	for (cornerId i = 0; i < corner_locations_.size(); i++)
	{
        if (IsConvexCorner(i)
            ...
            && !CornerAtDiagonalIntersection(i)
#endif
        )
        {
			Subgoal s;
...
#else   // Disallow moves between diagonal obstacles
void SubgoalAA::ComputeClearances()
{
    // Compute diagonal clearances. Inefficient, but since this is just precomputation, it does not matter.
    for (cornerId c = 0; c < corners_.size(); c++)
    {
        ...
        //for (direction d = 1; d <= 7; d +=2)
        {
            ...
            while(true)
            {
                next = GetCornerNeighbor(curr, d);
                if (next == NO_CORNER)
                {
                    SetClearance(c, d, clearance);
                    SetJumpTo(c, d, curr);
                    break;
                }
                else if (IsSubgoal(next))
                {
                    SetClearance(c, d, clearance+1);
                    SetJumpTo(c, d, next);
                    break;
                }
                clearance++;
    ...
    for (int x = 0; x < width_; x++)
        ...
        {
            if (cells_[x][y].is_traversable)
            {
                assert(cells_[x][y].northwest_corner < corner_locations_.size());
                assert(cells_[x][y].northeast_corner < corner_locations_.size());
                assert(cells_[x][y].southeast_corner < corner_locations_.size());
                assert(cells_[x][y].southwest_corner < corner_locations_.size());
            }
            else
            {
                assert(cells_[x][y].northwest_corner == NO_CORNER);
                assert(cells_[x][y].northeast_corner == NO_CORNER);
                assert(cells_[x][y].southeast_corner == NO_CORNER);
                assert(cells_[x][y].southwest_corner == NO_CORNER);
            }
        }
    ...
    for (direction d = 0; d <= 7; d++)
    {
        ...
        for (cornerId c = 0; c < corners_.size(); c++)
            if (GetJumpTo(c,d) == NO_CORNER || GetJumpTo(c,d) > corner_locations_.size())
            {
                std::cout<<c<<"-"<<d<<":\t"<<corner_locations_[c]<<"\tclearance:"<<GetClearance(c,d)<<"\tJump to:"<<GetJumpTo(c,d)<<std::endl;
                break;
            }
    }
#endif
...
#ifndef DISALLOW_MOVES_BETWEEN_DIAGONAL_OBSTACLES
void SubgoalAA::GetDirectHReachableSubgoals(const CornerLoc & from, std::vector<subgoalId> & subgoals)
{
...
void SubgoalAA::GetDirectHReachableSubgoals(const CornerLoc & from, std::vector<subgoalId> & subgoals)
{
    ...
    {
        clearance[d] = GetClearance(origin, d);
        cornerId jump_to = GetJumpTo(origin, d);

#ifdef ANY_ANGLE_ASSERTIONS
        assert(jump_to != NO_CORNER);
#endif

        if (IsSubgoal(jump_to) && clearance[d] != 0)
        {
            subgoals.push_back(corners_[jump_to].sg_id);
            clearance[d]--;
        }
    }
    ...
    for (direction d = 1; d <= 7; d+=2)
    {
        for (direction c = d-1; c <= d+1; c+=2)
        {
            ...
            direction card_dir = c % 8;
            int max_ext = clearance[card_dir];
            cornerId curr = origin;

            for (int i = 0; i < clearance[d]; i++) // Go along the diagonal, from each corner on the diagonal ..
            {
                curr = GetCornerNeighbor(curr, d);
                int card_clearance = GetClearance(curr, card_dir);

                if (card_clearance <= max_ext)
                {
                    max_ext = card_clearance;
                    cornerId jump_to = GetJumpTo(curr, card_dir);

                    #ifdef ANY_ANGLE_ASSERTIONS
                        assert(jump_to != NO_CORNER);
                    #endif

                    if (IsSubgoal(jump_to))
                    {
                        subgoals.push_back(corners_[jump_to].sg_id);
                        max_ext--;
                    }
...
// SEARCH
cost SubgoalAA::FindCornerLocPath(xyLoc from, xyLoc to, std::vector<CornerLoc> &path)
{
    switch (search_method_) {
...
cost SubgoalAA::FindSubgoalAnyAnglePath(xyLoc from, xyLoc to, std::vector<CornerLoc> & thePath, bool theta)
{
	thePath.clear();

	CornerLoc start = ToCornerLoc(from);
	CornerLoc goal = ToCornerLoc(to);
...
void SubgoalAA::ConnectStartAndGoalToGraph(
		CornerLoc & start, CornerLoc & goal, subgoalId & sgStart, subgoalId & sgGoal,
...
#ifdef ANY_ANGLE_ASSERTIONS
	assert(startCorner != NO_CORNER);
	assert(goalCorner != NO_CORNER);
#endif
...
cost SubgoalAA::ExtractPath(subgoalId start, subgoalId goal, std::vector<CornerLoc> & thePath)
{
	thePath.clear();
	// Extract path
...
// DISPLAY
#ifdef ANY_ANGLE_RUNNING_IN_HOG
void SubgoalAA::ProcessDebugLoc(const CornerLoc loc) {
    cornerId id = ToCornerId(loc);
    std::cout<<"Debug point: "<<loc<<" id: "<<id<<std::endl;
//    printf("Debug point: (%d, %d) - id: %d\n", loc.x, loc.y, id);

    //*
    if (IsSubgoal(id)) {
       std::cout<<"Subgoal!"<<std::endl;
       //const Subgoal* s = &subgoals_[ToSubgoalId(loc)];
       //for (unsigned int i = 0; i < s->neighbors.size(); i++) {
       //    CornerLoc loc2 = subgoals_[s->neighbors[i]].loc;
    ...
    //printf("Clearances: N(%u)\n", GetClearance(id, DIR_N));
    //*
    ...
    //printf("Neighbors: N(%u), NE(%u), E(%u), SE(%u), S(%u), SW(%u), W(%u), NW(%u)\n",
    //        GetCornerNeighbor(id, DIR_N),  GetCornerNeighbor(id, DIR_NE),  GetCornerNeighbor(id, DIR_E),  GetCornerNeighbor(id, DIR_SE),
    //        GetCornerNeighbor(id, DIR_S),  GetCornerNeighbor(id, DIR_SW),  GetCornerNeighbor(id, DIR_W),  GetCornerNeighbor(id, DIR_NW));
     //*/

    std::vector<subgoalId> neighbors;
    std::cout<<"Direct-h-reachable subgoals:";
    GetDirectHReachableSubgoals(loc, neighbors);
...
void SubgoalAA::VisualizeAlgorithm(const MapEnvironment *env)
{
    ...
    bool display_debug_loc = 1 && !(debug_loc_ == CornerLoc(10000, 10000));

    if (display_debug_loc) {
        env->SetColor(1,0,0);
        DrawPoint(env, debug_loc_);

        cornerId debug_id = ToCornerId(debug_loc_);

        /*
        if (debug_id != NO_CORNER)
        {
            env->SetColor(1,0,0);
            for (direction d = 0; d < 8; d++)
            {
                CornerLoc jump_to = ToCornerLoc(GetJumpTo(debug_id, d));
                DrawPoint(env, jump_to);
            }
        }
        /*/
